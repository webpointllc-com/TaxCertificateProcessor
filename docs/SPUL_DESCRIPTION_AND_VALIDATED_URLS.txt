...
================================
Updated: 2026-09-13T20:22:09.943Z
...
HOW SEARCH SHAPES WORK (examples)
---------------------------------
| You type              | What S-PUL does                                      |
| --------------------- | ---------------------------------------------------- |
| San Diego             | Unique county → San Diego, CA + locked Search/Base URL |
| San Diego, CA         | Exact county+state lock                              |
| Travis County TX      | Exact lock                                           |
| Chicago               | City alias → Cook, IL                                |
| LA County / SD County | Abbrev alias → Los Angeles / San Diego, CA           |
| Jackson               | Multi-state → ask which state (do not URL-lock)      |
| pay taxes Miami       | Intent = pay; place alias → Miami Dade, FL           |
| (unknown place)       | Honest miss + clarify county + state                 |

WHERE IT LIVES
--------------
- Code / this deploy branch: search-spul-minimal (Render Blueprint ready)
- Registry: spul/data/search-index.json + counties-export.csv
- LLM guide Path B: POST /api/guide (and /api/chat) — narrates around locked
  registry truth only; optional Groq is narrative sugar, not URL inventing
- Live host (when deployed): https://search-spul-minimal.onrender.com

BRAND / COMPANY CONTEXT
-----------------------
Webpoint / TaxCert.ai sell B2B access to jurisdiction search (company accounts,
Google OAuth seats, Stripe billing) — not Squarespace Members paywalls, not
RAG chat over tax docs. S-PUL is the answering registry product.

METHODOLOGY NOTE (URL validation, 2026-09-13)
---------------------------------------------
Hybrid revalidation A + B + C on the full Extractor export
(spul/data/counties-export.csv on branch s-pul-front_end-Betaspul-minimal-deploy-55de).

Method A — HTTP HEAD, then GET; follow ≤4 redirects; 12s timeout.
  • 200–399, and bot/auth walls 401/403/405/429, plus soft gateway 502/503
    → validated-true (host alive).
  • 404/410, NXDOMAIN, InvalidURL, google.com placeholders → dead.
  • Timeout / TLS / other hard failures → uncertain (keep active in registry
    to avoid false-negative wipeouts; NOT listed as validated-true).

Method B — Prefer Search URL over Base (already how primary_search_url is
chosen); skip / kill google placeholders.

Method C — Redirect follow; sample title/form sniff (n=150 Search-first);
cross-check host vs state+county (soft; shared vendors like governmax /
tyler / mptsweb are not mismatches). Hard host mismatches this run: 0.

DeepShake: NOT RUN. Cloud Linux VM has no /Volumes/T7 and no connected Mac
self-hosted worker. Mac handoff: bash scripts/deepshake-hunt-mac.sh then
cursor worker start. Do not block deploys on DeepShake.

Judgment — is registry + health + alias the right approach vs heavier extraction?
YES for shipping S-PUL. The product promise is "locked official URL + confidence
or honest miss." Registry + HTTP health + local aliases delivers that without
inventing sites. Heavier live extraction / DeepShake / full HTML triangulation
is the right next layer for *repairing* dead URLs and raising confidence — not
a substitute for the locked registry on the request path. Full page extraction
of every county on every query would be slow, fragile, and look like abuse.

COUNTS (this revalidate)
------------------------
Listed jurisdictions:     2062
With primary_search_url:  2055
Validated-true (HTTP):    1437
Dead (clear):             368
Uncertain (timeout/TLS/…):250
Registry active after write-back (validated-true + uncertain kept): 1681
Flipped inactive (clear dead wins): 288
Flipped active: 0
Sniff sample tax/form hits: 73/150
Gated-but-alive (403/429/…): 241
...
END OF VALIDATED-TRUE LIST
Total validated-true rows: 1437
...
Clipboard tip for Bill: open the .txt → Select All → Copy
(xclip may work in this environment; Mac pbcopy / computerUse not available here.)
